...
export function Layout({ children }: { children: React.ReactNode }) {
  const user = useAppStore((state) => state.user); const setUser = useAppStore((state) => state.setUser); const queryClient = useQueryClient(); const navigate = useNavigate()
  const currentUser = useQuery({ queryKey: ['me'], queryFn: client.me, retry: false })
  useEffect(() => { if (currentUser.data) setUser(currentUser.data.user); if (currentUser.isError) setUser(null) }, [currentUser.data, currentUser.isError, setUser])
