...
api.interceptors.request.use((config) => {
  if (!['get', 'head'].includes(config.method || 'get') && csrfToken)
    config.headers['x-csrf-token'] = csrfToken
  return config
})
async function csrf() {
  if (!csrfToken) csrfToken = (await api.get('/auth/csrf')).data.data.token as string
}
...
export const client = {
  ...
  login: async (values: { email: string; password: string }) => {
    await csrf()
    return data<{ user: User }>(api.post('/auth/login', values))
  },
  register: async (values: { name: string; email: string; password: string }) => {
    await csrf()
  ...
  trips: (params: Record<string, string | boolean | number | undefined>) =>
    data<TripCard[]>(api.get('/buses', { params })),
  tripsPage: (params: Record<string, string | boolean | number | undefined>) =>
    paginated<TripCard[]>(api.get('/buses', { params })),
  trip: (busId: string, tripId: string) =>
