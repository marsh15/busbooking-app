...
function Protected({ children }: { children: React.ReactNode }) { const user = useAppStore((state) => state.user); const authReady = useAppStore((state) => state.authReady); const location = useLocation(); if (!authReady) return <main className="page-shell"><div className="skeleton detail-skeleton" aria-busy="true" aria-label="Restoring your session" /></main>; return user ? <>{children}</> : <Navigate to="/login" state={{ from: location.pathname }} replace /> }
