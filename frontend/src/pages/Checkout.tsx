...
export function CheckoutPage() {
  ...
  const form = useForm<CheckoutValues>({ resolver: zodResolver(schema) as Resolver<CheckoutValues>, defaultValues: { passengers: selected.map((seatNumber) => ({ seatNumber, name: '', age: 25 })) } }); const fields = useFieldArray({ control: form.control, name: 'passengers' }); const { reset } = form
  useEffect(() => { reset({ passengers: selected.map((seatNumber) => ({ seatNumber, name: '', age: 25 })) }) }, [selected, reset])
