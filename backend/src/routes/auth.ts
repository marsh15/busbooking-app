...
authRouter.post('/register', asyncRoute(async (request, response) => {
  ...
  try {
    const user = await prisma.user.create({ data: { name: input.name, email: input.email.toLowerCase(), passwordHash: await argon2.hash(input.password, { type: argon2.argon2id }) } })
    issueSession(response, user.id)
    response.status(201).json({ data: { user: publicUser(user) } })
  } catch (error) {
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') throw new ApiError(409, 'EMAIL_IN_USE', 'An account already exists for this email.')
    throw error
  }
}))
...
authRouter.get('/me', requireAuth, asyncRoute(async (request: AuthRequest, response) => {
