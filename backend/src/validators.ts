...
export const bookingSchema = z.object({
  ...
  passengers: z
    .array(
      z.object({
        seatNumber: z.string().regex(/^\d[A-D]$/),
    ...
    .max(6)
    .refine(
      (passengers) => new Set(passengers.map((passenger) => passenger.seatNumber)).size === passengers.length,
...
export const paginationSchema = z.object({
  ...
  pageSize: positiveInt(50).default(20),
})
