...
beforeEach(async () => {
  ...
  await prisma.user.deleteMany({ where: { email: { not: 'demo@voyagebus.in' } } })
  await prisma.seat.updateMany({
    where: {
      trip: {
        travelDate: { in: [new Date(`${seedDate}T00:00:00.000Z`), new Date('2030-01-11T00:00:00.000Z')] },
      },
    },
...
describe.sequential('VoyageBus API with MySQL persistence', () => {
  ...
  it('reruns the seed without duplicates or resetting seat state', async () => {
    ...
    await owner.agent
      ...
      .send({ tripId: trip.id, passengers: [{ seatNumber: '1A', name: 'Seed Rider', age: 29 }] })
      .expect(201)
  ...
  it('allows exactly one concurrent owner to claim a seat', async () => {
    ...
    expect(
      await prisma.booking.count({ where: { tripId: trip.id, seatNumber: '1A', status: 'ACTIVE' } }),
    ).toBe(1)
  })
  ...
  it('retrieves the same booking and PNR after reconnecting the database client', async () => {
    ...
    const created = await owner.agent
      .post('/api/bookings')
      .set('x-csrf-token', owner.csrf)
      .send({ tripId: trip.id, passengers: [{ seatNumber: '4A', name: 'Restart Rider', age: 40 }] })
      .expect(201)
  ...
  it('rolls back every seat in a failed multi-seat claim', async () => {
    ...
    await first.agent
      ...
      .send({ tripId: trip.id, passengers: [{ seatNumber: '1B', name: 'First Rider', age: 30 }] })
      .expect(201)
    await second.agent
      .post('/api/bookings')
      ...
      .send({
        tripId: trip.id,
        passengers: [
          { seatNumber: '1A', name: 'Second Rider', age: 31 },
          { seatNumber: '1B', name: 'Third Rider', age: 32 },
        ],
      })
      .expect(409)
  ...
  it('persists a group and supports partial then final cancellation', async () => {
    ...
    const created = await owner.agent
      ...
      .send({
        tripId: trip.id,
        passengers: [
          { seatNumber: '2A', name: 'First Rider', age: 35 },
          { seatNumber: '2B', name: 'Second Rider', age: 36 },
        ],
      })
      .expect(201)
  ...
  it('handles concurrent duplicate cancellation without releasing a seat twice', async () => {
    ...
    const created = await owner.agent
      .post('/api/bookings')
      .set('x-csrf-token', owner.csrf)
      .send({ tripId: trip.id, passengers: [{ seatNumber: '3A', name: 'Concurrent Rider', age: 38 }] })
      .expect(201)
