...
export function parseSearchFallback(query: string): ParsedSearch {
  ...
  return {
    ...
    timePreference: clean.includes('morning')
      ? 'morning'
    ...
    maxPrice: max ? Number(max[1]) : null,
    provider: 'fallback',
    warnings,
  }
}
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        input: `Extract only supported VoyageBus search fields. Today in Asia/Kolkata is ${istDate()}. Resolve today/tomorrow to YYYY-MM-DD. Only use these cities: ${cities.join(', ')}. Never invent availability. User query: ${query}`,
        text: {
          format: {
            type: 'json_schema',
            name: 'voyagebus_search',
            strict: true,
            schema: {
              type: 'object',
              additionalProperties: false,
              properties: {
                source: { type: ['string', 'null'], enum: [...cities, null] },
                destination: { type: ['string', 'null'], enum: [...cities, null] },
                date: { type: ['string', 'null'] },
                timePreference: {
                  type: ['string', 'null'],
                  enum: ['morning', 'afternoon', 'evening', 'night', null],
                },
                busType: { type: ['string', 'null'], enum: ['SLEEPER', 'SEATER', null] },
                isAc: { type: ['boolean', 'null'] },
                maxPrice: { type: ['integer', 'null'] },
              },
              required: ['source', 'destination', 'date', 'timePreference', 'busType', 'isAc', 'maxPrice'],
            },
          },
        },
      }),
